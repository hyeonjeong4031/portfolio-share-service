...
//1. 자격증 추가
router.post("/create", login_required, async (req, res, next) => {
  try {
...
//2. 자격증 정보 전체 조회
router.get("/certificatelist", login_required, async (req, res, next) => {
  try {
    const user_id = req.currentUserId;
...
//3. 자격증 정보 수정
router.put("/edit", login_required, async (req, res, next) => {
  try {
  } catch (error) {
    next(error);
  }
...
//4. 자격증 정보 삭제
router.delete("/delete", login_required, async (req, res, next) => {
  try {
  } catch (error) {
    next(error);
  }
